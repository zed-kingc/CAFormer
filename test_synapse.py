...
from lib.networks import PVT_GCASCADE, MERIT_GCASCADE, PVT_UPGCASCADE, Unet
...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        logging.info('idx %d case %s mean_dice %f mean_hd95 %f, mean_jacard %f mean_asd %f mean_iou %f' % (i_batch, case_name, np.mean(metric_i, axis=0)[0], np.mean(metric_i, axis=0)[1], np.mean(metric_i, axis=0)[2], np.mean(metric_i, axis=0)[3], np.mean(metric_i, axis=0)[4]))
    metric_list = metric_list / len(db_test)
    for i in range(1, args.num_classes):
        logging.info('Mean class (%d) %s mean_dice %f mean_hd95 %f, mean_jacard %f mean_asd %f mean_iou %f' % (i, classes[i-1], metric_list[i-1][0], metric_list[i-1][1], metric_list[i-1][2], metric_list[i-1][3], np.mean(metric_i, axis=0)[4]))
    performance = np.mean(metric_list, axis=0)[0]
    mean_hd95 = np.mean(metric_list, axis=0)[1]
    mean_jacard = np.mean(metric_list, axis=0)[2]
    mean_asd = np.mean(metric_list, axis=0)[3]
    mean_iou = np.mean(metric_list, axis=0)[4]
    logging.info('Testing performance in best val model: mean_dice : %f mean_hd95 : %f, mean_jacard : %f mean_asd : %f mean_iou %f' % (performance, mean_hd95, mean_jacard, mean_asd, mean_iou))
...
if __name__ == "__main__":
    ...
    args.exp = str(args.encoder) + '_MUTATION_w3_7_Run1_' + dataset_name + str(args.img_size)#+ '_cc_th' +str(args.cc_th) #HR_CASCADERun3_
    ...
    snapshot_path = snapshot_path+'_bs'+str(24)
    ...
    elif args.encoder=='UP':
        net = PVT_UPGCASCADE(n_class=args.num_classes, img_size=args.img_size, k=11, padding=5, conv='mr', gcb_act='gelu', skip_aggregation=args.skip_aggregation)
    elif args.encoder =='Unet':
        net = Unet(n_class=args.num_classes, img_size=args.img_size, k=11, padding=5, conv='mr', gcb_act='gelu', skip_aggregation=args.skip_aggregation)
    else:
