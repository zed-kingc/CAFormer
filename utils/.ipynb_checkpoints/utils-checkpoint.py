...
def calculate_iou(pred, true):
    # 确保pred和true是相同形状的二值数组
    intersection = np.logical_and(pred, true)
    union = np.logical_or(pred, true)
    iou = (np.sum(intersection) + 1e-6) / (np.sum(union) + 1e-6)  # 防止除以零
    return iou
def calculate_metric_percase(pred, gt):
    ...
    if pred.sum() > 0 and gt.sum()>0:
        ...
        return dice, hd95, jaccard, asd, iou
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                if x != patch_size[0] or y != patch_size[1]:
                    ...
                    pred = out
                prediction[ind] = pred[:, :, 0]
                
                # 一个输出
                # P = net(input)
                # out = torch.argmax(torch.softmax(P[0], dim=0), dim=0)  # 直接使用P的第一个元素
                # out = out.cpu().detach().numpy()
                # if x != patch_size[0] or y != patch_size[1]:
                #     pred = zoom(out, (x / patch_size[0], y / patch_size[1]), order=0)
                # else:
                ...
                #下面是对的
                # fig_gt = overlay_masks(image[ind, :, :], np.stack(masks, -1), labels=mask_labels, return_type="mpl")
                # fig_pred = overlay_masks(image[ind, :, :], np.stack(preds_o, -1), labels=mask_labels, return_type="mpl")
